...
/// miscomp-issue parsed startup args.
pub fn args() -> &'static MiscompIssueArgs {
    &CFG
}

impl MiscompIssueArgs {
...
// called by miscomp-issue/cli.rs
pub fn init_args(cfg: MiscompIssueArgs) {
    if lazy_static_init(&CFG, cfg).is_err() {
        panic!("shared::env::args is already inited");
    }
}
lazy_static! {
    static ref CFG: MiscompIssueArgs =
        panic!("shared::env::args not inited, only use after miscomp_issue::cli");
}

// called by miscomp-issue/config.rs
